...
@section('breadcrumb')
    @parent
    <li class="breadcrumb-item"><a href="{{ route('bmasuk.index') }}">Barang Keluar</a></li>
...
<div class="row">
    <div class="col-lg-12">
        <form action="{{ route('bkeluar.store') }}" method="POST">
            @csrf
            <div class="card">
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <select name="id_daftar_barang" class="form-control" id="id_daftar_barang" required>
                            @foreach ($daftarBarang as $barang)
                                <option value="{{ $barang->id }}">{{ $barang->kd_barang }} - {{ $barang->nama }}</option>
                            @endforeach
                        </select>
